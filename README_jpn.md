...
- **状態の所有権とプロジェクト間の関係を再決定するのはこのリポジトリの
  ...
  [docs/OPS_INTEGRATION.md](docs/OPS_INTEGRATION.md) は、非公開の
  ソフトウェア準備計画自身の状態所有権表と、17のプロジェクト間関係を
  必須/任意/開発専用に分類したものを、再解釈せずにそのまま再現している。
...
│   ├── ARCHITECTURE.md     # 目的、作業モード、初期範囲、ディスク（計画13.1/13.3-13.5）
│   └── OPS_INTEGRATION.md  # 17関係マップ＋所有権表（計画13.2.1/13.2.2）
├── images/                # メディアとアプリアイコン
├── tools/
