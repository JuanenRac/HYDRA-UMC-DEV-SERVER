...
(`migrate` - see `MIGRATION_FROM_PC.md`); and DS04's bounded task recipe
and isolated workspace runner (`task` - see `WORKSPACE_AND_RUNNER.md`).
DS01-DS03 only read and describe. DS04 is the first delivery that
executes a subprocess - and only an allow-listed command, in a per-task
